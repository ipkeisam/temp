...
def encrypt_key(keyid,wrapping_key,okta_token,environment):
    ...
    try: 
        ...
        print("Error encrypting key material. Error : {}".format(e))
        raise
    finally:
...
def lambda_handler(event,context):
    ...
    qadatatype = event['qadatatype'].lower()
    ...
    environment_mapping = {'na':'oz-dev', 'foundation':'az-inf', 'sbx':'oz-inf', 'dev':'oz-dev', 'qa':qa_environment_default, 'prd':'cpz-prd'}
    ...
    kmsregions = ["us-west-1","us-east-1","us-west-2","us-east-2"]
    keyresources = ["s3","rds","vol"]
    keydescription = ["default KMS for s3","default KMS for rds","default KMS for ebs and efs"]
    length = len(keyresources)
    for i in range(length):
        resource = keyresources[i]
        description = keydescription[i]
        print(resource)
        print(description)
        ...
        #Create key policy for the specific resource
            default_ebs_encryption_successful[deployregion] = False
            keypolicy = ''
            if "vol" in resource:
            ...
            #Create CMK
            (keyid,keyarn,is_cmk_created) = create_cmk(credentials,description,keypolicy,environment,deployregion)
    ...
    event['are_kms_keys_created'] = are_kms_keys_created
    event['default_ebs_encryption_successful'] = default_ebs_encryption_successful
    print(event['are_kms_keys_created'])
    print(event['default_ebs_encryption_successful'])
    return event